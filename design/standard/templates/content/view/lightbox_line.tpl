...
{if $content_object}
    ...
    {$content_object.name|wash()} ({$content_object.class_name|wash()})
...
{if $content_object.assigned_nodes|count()|gt( 0 )}

    <ul>

    {foreach $content_object.assigned_nodes as $node}
...
{/if}
  ...
  <div class="published">{cond( $content_object, $content_object.published|l10n( 'date' ) )}</div>
 </div>
 <div class="actions">
  <form action={'/content/action'|ezurl()} method="post">
   <input type="hidden" name="ContentObjectID" value="{$object_id}" />
   <input type="hidden" name="ContentNodeID"   value="{cond( $content_object, $content_object.main_node.node_id )}" />
   <input type="hidden" name="ItemID"          value="{$object_id}" />
   <input type="hidden" name="ItemType"        value="1" />

{* Add to basket *}

{if and( $content_object, $can_use_basket )}
...
{if $lightbox.can_edit}
   ...
   <input type="image" src={'action_lightbox_delete_medium.png'|ezimage()} title="{'Use this button to delete the item from this lightbox'|i18n( 'design/standard/view/lightbox_line' )}" name="RemoveFromLightboxAction" alt="{'Remove'|i18n( 'design/standard/view/lightbox_line' )}" />
    ...
    {if and( $content_object, is_array( $lightboxList ), $lightboxList|count()|gt( 1 ) )}
